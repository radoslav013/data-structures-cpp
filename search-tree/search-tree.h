...
class SearchTree {
    ...
    protected: // local utilities
        ...
        TPos root() const { return T.root().getLeft(); }  // get virtual root
...
typename SearchTree<E>::Iterator SearchTree<E>::begin() {
    ...
    while (v.isInternal()) v = v.getLeft(); // find leftmost node
...
typename SearchTree<E>::TPos SearchTree<E>::finder(const K& k, const TPos& v) {
    ...
    if (k < ent.key()) return finder(k, v.getLeft()); // search left subtree
    else if (ent.key() < k) return finder(k, v.getRight()); // search right subtree
...
typename SearchTree<E>::TPos SearchTree<E>::inserter(const K& k, const V& x) {
    ...
    while (v.isInternal()) // key already exists?
        v = finder(k, v.getRight()); // look further
...
typename SearchTree<E>::TPos SearchTree<E>::eraser(TPos& v) {
    TPos w;
    if (v.getLeft().isExternal()) w = v.getLeft(); // remove from left
    else if (v.getRight().isExternal()) w = v.getRight(); // remove from right
    else { // both internal?
        w = v.getRight(); // go to right subtree
        do { w = w.getLeft(); } while (w.isInternal()); // get leftmost node
        TPos u = w.parent();
